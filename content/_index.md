+++
title = "Alexander Sakhnov"
description = "Mechanical & Electronic Engineer | Additive Manufacturing | CAD | AI"
_build = { list = false }
+++

# Alexander Sakhnov
**Mechanical & Electronic Engineer**

I am an engineer driven by curiosity, creativity and a strong desire to build real, functional solutions.
My background combines **Mechanical Engineering**, **Electronic Engineering**, **Additive Manufacturing**, and **Industrial Automation**, allowing me to approach problems from both a technical and practical perspective.

Over the years, I have worked with a wide range of technologies:
from **3D printing with clay, concrete, FDM and SLA**, to **robotics**, **renewable energy systems**, **CAD design**, and **AI‑assisted workflows**.
I enjoy designing, prototyping and improving systems that connect the digital and physical world.

---

## About me

- Born in 1997, currently based in Arenys de Mar (Barcelona)
- Native Russian, fluent in Spanish and Catalan, B2 English
- Passionate about engineering, innovation and hands‑on creation
- Strong interest in additive manufacturing, industrial automation and AI
- Creative, resolutive, responsible and always eager to learn
- Driver’s license (B) and full availability

---

## Academic Background

- **Double Degree in Mechanical Engineering + Electronic & Industrial Automation Engineering**
  Universitat Politècnica de Catalunya (UPC – EEBE)

- **Postgraduate in Artificial Intelligence: Machine & Deep Learning**
  Universitat Politècnica de Catalunya

- **MBA – Technology & Industrial Management**
  Universitat Politècnica de Catalunya

- Additional training in physics, English immersion programs and technical workshops.

---

## Professional Experience

- **Technical Instructor – EMT Granollers**
  Teaching and guiding students in industrial and technological subjects.

- **Research Intern – UPC**
  Study and characterization of 3D‑printed materials.

- **Receptionist – Hotel JET**
  Customer service and administrative tasks.

---

## Technical Skills

- Additive manufacturing (FDM, SLA, clay, concrete)
- CAD design (SolidWorks, AutoCAD, Inventor)
- 3D scanning and prototyping
- Solar and wind energy installations
- Industrial electrical systems and machine maintenance
- Robotics and custom robot design
- Python, G‑code, VHDL
- AI tools for engineering and design

---

If you want to explore my work, you can visit:

- **Projects** → Real engineering and personal builds
- **Work** → Professional experience
- **Studies** → Academic background and certifications